...
複雑なシステムをテストする必要がある場合には、手作業のテスターを退屈にし、時間がかかり、そして実行するのが誤りである複雑なステップを繰り返さなければならないという困難な作業を自動化することからの大きな利益がある。
...
自動化のための試験環境は、柔軟であり、経時的にＳＵＴに生じるであろう変化に適応可能である必要がある。 これには、自動化に関する問題を迅速に診断して修正する機能、自動化コンポーネントを保守することの容易さ、および自動化環境に新しい機能およびサポートを追加できる機能が含まれます。 これらの属性は、gTAAの全体的な設計と実装に不可欠な要素です。
...
多くの場合、初期条件を設定する前にテストを実行することはできません。 これらの条件は、試験する元の正しいデータベースまたは試験データセットを選択すること、あるいは初期値またはパラメータを設定することを含み得る。 テストの前提条件を確立するために必要なこれらの初期化手順の多くは自動化できます。 テストを実行する前にこれらのステップを見逃すことができない場合は、これにより信頼性と信頼性の高いソリューションが可能になります。回帰テストは自動化に変換されるので、これらの前提条件は自動化プロセスの一部である必要があります。
...
SUTに新しい機能が導入されると、テスターはこれらの新機能および対応する要件に対して新しいテストを開発する必要があります。 TAEは、ドメスティックな経験を持つテスト設計者からのフィードバックを求め、現在のTASが新しい機能のニーズを満たすかどうかを判断する必要があります。 この分析には、使用される既存のアプローチ、サードパーティの開発ツール、使用されるテストツールなどが含まれますが、これらに限定されません。
...
既存の要件が変更された場合、これらの要件を検証するテストケースを更新するための努力は、プロジェクトスケジュールの一部である必要があります（作業詳細構造）。 要件からテストケースまでのトレーサビリティは、どのテストケースを更新する必要があるかを示します。 これらの更新は全体的な計画の一部であるべきです。
...
確認テストは、報告された問題に対処するコード修正に従って実行されます。 テスターは通常、欠陥がもはや存在しないことを確認するために欠陥を複製するのに必要なステップをフォローする。
...
自動確認試験を追跡することは、欠陥を解決するのに費やされた時間およびサイクル数のさらなる報告を可能にする。
